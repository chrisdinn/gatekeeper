...
Be sure to use the session middleware when building a Rack app, or to [+enable :sessions+] when using Sinatra as Gatekeeper relies on sessions
...
When using Rails, you should be sure *not* to [+enable sessions+] in your Sinatra metal. Rails takes care of the session. If you re-enable, you'll overwrite what Rails
