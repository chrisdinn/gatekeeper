...
run [+script/generate metal sso+]. Inside, you should have:
...
that, simply include [+Gatekeeper::Helpers::Authentication+] in your app. You can then use the following methods:
	...
	[current_user]	Will return [+nil+] or the current user's id, depending on whether or not the user is logged in.
	[is_admin?]		Is this user a Hot Ink admin? You may have some tasks that only admin users can do.
	[is_manager_of?(hotink_account_id)]		Will return true if the user is a manager of the account who's id you passed in.
